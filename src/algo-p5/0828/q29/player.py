...
class Player:
    ...
    def show_status(self):
        ...
        #「経験値を(self.exp)獲得しています。レベルは(self.level)です。」を表示してください。
        print("経験値を" + self.exp + "獲得しています。レベルは" + self.level + "です。")
    ...
    def battle(self, enemy):
        ...
        #経験値を1獲得します。self.expに1を足して(self.expに再代入して)ください。
        self.exp += 1
        ...
        if self.exp % 10 == 0:
            #経験値を10獲得するごとにレベルアップします。self.expを10で割った余りが0の場合、self.levelに1を足して(self.levelに再代入して)ください。
            self.level += 1

            #最大HPを10上げます。self.max_hpに10を足してください.
            self.level + 10

            #敵に与える最小ダメージを1上げます。self.min_damageに1を足してください。
            self.min_damage + 1

            #敵に与える最大ダメージを2上げます。self.max_damageに2を足してください。
            self.max_damage + 2

            #「(self.name)のレベルが(self.level)に上がった!最大HPが(self.max_hp)になりました。」を表示してください。
            print(self.name + "のレベルが" + str(self.level) + "に上がった！最大HPが" + self.max_hp + "になりました。")
    ...
    def poisoned_in_swamp(self):
        ...
        if self.hp > 0:
            # まだプレイヤーが生きている
            print(self.name + "は20のダメージを受けた!現在のHPは" + int(self.hp) + "です。")
        else:
            # プレイヤーが死んでしまった
