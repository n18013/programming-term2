...
class Player:
    ...
    def battle(self, enemy):
        ...
        while enemy.hp > 0:
            ...
            if cmd_num == "1":
                ...
                # 薬草を使う
                self.use_plants()
            elif cmd_num == "9" and enemy.__class__.__name__ != "BossChara":
                # 逃げる(ボスキャラ以外)
    ...
    #pick_up_plant()メソッドを定義します。引数はselfのみです。
    def pick_up_plant(self):
        ...
        なし
        """
        #「イベント発生！薬草をみつけた!」を表示してください。
        print("イベント発生！薬草を見つけた！")


        #self.plant_numsに1を足して(再代入して)ください。
        self.plant_nums += 1


        #「(self.name)は薬草を手に入れた!現在薬草を(self.plant_nums)枚持っています。」を表示してください。
        print(self.name + "は薬草を手に入れた！現在薬草を" + str(self.plant_nums) + "枚持っています。")
