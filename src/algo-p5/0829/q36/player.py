...
from boss_chara import BossChara
...
class Player:
    ...
    def battle_vs_boss(self):
        ...
        なし
        """
        # "さちさち"という名前のBossCharaクラスのオブジェクトを生成
        boss = BossChara("さちさち")
    ...
    def battle(self, enemy):
        ...
        while enemy.hp > 0:
            ...
            #この後inputの引数に与えるガイド文字列をキャラクターによって分岐します。
            if enemy.__class__.__name__ == "BossChara":
                #ボスキャラの場合(enemy.__class__.__name__ == "BossChara")、変数cmd_guideに、"1:攻撃する、3:薬草を使う>> "を代入する。
                cmd_guide = ("1:攻撃する、3:薬草を使う>>")

            
                #ボスキャラ以外の場合(else)、変数cmd_guideに、"1:攻撃する、3:薬草を使う、9:逃げる>> "を代入する。
            else:
                cmd_guide = ("1:攻撃する、3:薬草を使う、9:逃げる>>")



            # バトル中のコマンドを選択させる
            #下記input()関数の引数を、変数cmd_guideに変更してください
            cmd_num = input(cmd_guide)
            ...
            if cmd_num == "1":
                ...
                enemy.__class__.__name__ !="BossChara"
                print("ボス戦では逃げられない")
                #直前の行の条件(elif句)に、enemy.__class__.__name__ != "BossChara"をand条件として追加してください。(ボス戦では逃げられない)
                # 逃げる
        ...
        # 経験値を獲得
        #ボスキャラ戦以外の場合(enemy.__class__.__name__ != "BossChara")のみself.get_exp()を呼び出してください。
            
        
        if enemy.__class__.__name != "BossChara":
            self.got_exp()
